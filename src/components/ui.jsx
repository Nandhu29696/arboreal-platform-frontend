...
export function StatCard({ label, value, icon: Icon, color = 'var(--accent)', sub }) {
  return (
    <Card style={{ display: 'flex', alignItems: 'flex-start', gap: '1rem' }}>
      ...
      <div>
        <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontWeight: 500, marginBottom: 2, fontFamily: 'var(--font-display)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{label}</div>
...
export function Btn({ children, onClick, variant = 'primary', size = 'md', disabled, type = 'button', style }) {
  const sizes = { sm: '0.42rem 0.85rem', md: '0.55rem 1.15rem', lg: '0.75rem 1.5rem' };
  const base = {
    display: 'inline-flex', alignItems: 'center', gap: '0.4rem',
    border: '1px solid transparent', borderRadius: 8,
    fontFamily: 'var(--font-display)', fontWeight: 600,
    fontSize: size === 'sm' ? '0.78rem' : size === 'lg' ? '0.95rem' : '0.85rem',
    padding: sizes[size], cursor: disabled ? 'not-allowed' : 'pointer',
  ...
  const variants = {
    ...
    secondary: { background: 'transparent', color: 'var(--text-secondary)', borderColor: 'var(--border)' },
    danger: { background: 'rgba(248,113,113,0.1)', color: 'var(--danger)', borderColor: 'rgba(248,113,113,0.3)' },
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <h3 style={{ fontFamily: 'var(--font-display)', fontSize: '1rem', color: 'var(--text-primary)' }}>{title}</h3>
...
export function FormRow({ children, cols = 2 }) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: '0.85rem', marginBottom: '0.85rem' }}>
      {children}
    </div>
  );
}
...
export function Field({ label, children, required }) {
  return (
    <div>
      <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 600, color: 'var(--text-muted)', marginBottom: '0.35rem', fontFamily: 'var(--font-display)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
...
export function Pagination({ page, total, limit, onPage }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', justifyContent: 'flex-end', marginTop: '1rem' }}>
      ...
      <Btn variant="secondary" size="sm" disabled={page >= pages} onClick={() => onPage(page + 1)}>
...
export function Empty({ message = 'No records found' }) {
  return (
    <div style={{ textAlign: 'center', padding: '3rem', color: 'var(--text-muted)' }}>
      ...
      <div style={{ fontFamily: 'var(--font-display)' }}>{message}</div>
    </div>
  );
}
...
export function PageHeader({ title, subtitle, action }) {
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
      <div>
        <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '1.4rem', color: 'var(--text-primary)', marginBottom: 2 }}>{title}</h2>
